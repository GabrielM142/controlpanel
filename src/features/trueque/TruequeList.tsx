...
function readLocal():Request[]{try{const raw=localStorage.getItem('side.trueque.local');if(!raw)return [];const arr=JSON.parse(raw);return Array.isArray(arr)?arr:[];}catch{return [];}}
...
export default function TruequeList(){
 ...
 return <>
  ...
  <p className="legend-note" style={{marginTop:14}}><b>Trueque Labs.</b> Los módulos entregados se despliegan como paquetes independientes dentro del ERP existente. Los tokens se consumen por captación y análisis; no se acumulan mes a mes.</p>
 </>;
}
